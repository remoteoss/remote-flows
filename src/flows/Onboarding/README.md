...
| `isSubmitting`             | `boolean`                                                                                                                       | Loading state indicating if the onboarding mutation (or the job title eligibility check) is in progress                                                                                                                             |
...
| `checkJobTitleEligibility` | `(values: FieldValues) => Promise<void>`                                                                                        | Runs the job title eligibility check when the `job_title_eligibility` feature is enabled and the role fields are filled. The prebuilt form calls it on blur; it also runs when entering contract details and before submitting them |
