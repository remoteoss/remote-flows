...
describe('useContractorOnboarding select country step', () => {
  ...
  const renderSelectCountryStep = async (
    options: ContractorOnboardingFlowProps['options'] = {},
  ) => {
    const { result } = renderHook(() => useContractorOnboarding({ options }), {
  ...
  it('applies the consumer jsfModify on top of the country options', async () => {
    const result = await renderSelectCountryStep({
      jsfModify: {
        select_country: {
          fields: { country: { title: 'Where does the contractor live?' } },
        },
      },
    });

    expect(result.current.fields[0].label).toBe(
      'Where does the contractor live?',
    );
    expect(result.current.fields[0].options).toEqual(expectedOptions);
  });
