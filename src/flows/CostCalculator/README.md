...
| `handleValidation` | `(values) => Promise<{ formErrors, yupError }>`                                                                      | Validates the form values against the JSON schemas (static fields + region fields) and returns the errors per field name |
| `validationSchema` | `yup.Schema`                                                                                                         | **Deprecated**, use `handleValidation`. Yup schema whose `validate` delegates to `handleValidation`                      |
