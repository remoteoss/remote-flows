...
### Phase 4 — Wire the `jsfModify` Component override into Onboarding's `contract_details` step (done)

- Done: `daily_schedule` added to `contractDetailsCustomFields.fields` in `Onboarding/hooks.tsx`, alongside the existing `annual_gross_salary`/`equity_compensation` entries, injecting `x-jsf-presentation.Component`.
- **Correction (verified against the actual code, not just the plan's earlier prose): mirror `paid_time_off_info`'s real wiring exactly — no deviation needed.** An earlier revision of this plan claimed `paid_time_off_info`'s wiring lets a consumer-supplied `Component` fully replace the field (bypassing `PaidTimeOffContainer`), and proposed deviating from that for `daily_schedule`. That claim was wrong: reading `Termination/hooks.tsx:206-222` directly shows `PaidTimeOffContainer` is **always** rendered — the captured consumer `Component` (`options?.jsfModify?.fields?.paid_time_off_info?.['x-jsf-presentation']?.Component`) is passed to `PaidTimeOffContainer`'s `render` prop as a conditional branch (`render={(props) => CustomComponent ? <CustomComponent {...props} /> : <PaidTimeOff {...props} />}`), never used as the field's own `Component`. This is exactly the shape `daily_schedule` needs — the container (and therefore `useDailyScheduleEditForm`) always runs, and a consumer's override is just the presentational layer. (The pattern that _does_ bypass a container is `annual_gross_salary`'s — but it has no container to begin with, so there's nothing to bypass.) So: capture `options?.jsfModify?.fields?.daily_schedule?.['x-jsf-presentation']?.Component` first, then unconditionally set `x-jsf-presentation.Component` to a function that renders `<DailyScheduleContainer {...fieldProps} render={(props) => CustomComponent ? <CustomComponent {...props} /> : <DailySchedule {...props} />} />`. A consumer building fully custom UI calls `useDailyScheduleEditForm` themselves inside their `CustomComponent`, the same way `DailySchedule.tsx` does.
...
- ~~Check/extend `useOnboardingJsfV1ContractDetails.test.tsx` and sibling contract-details tests for a `daily_schedule`-bearing fixture.~~ Done in Phase 4: `useOnboardingDailySchedule.test.tsx` + `tests/fixtures/contractDetails/v1-daily-schedule.ts` cover the flag on/off, consumer-override, and no-op-without-the-field cases at the `useOnboarding` wiring level. Still open: exercising this through the full `OnboardingFlow` step machine (not just the hook + a directly-rendered field), which is what the "full flow" bullet above covers.
- ~~Confirm the `jsfModify` field entry is a no-op for schemas/countries without a `daily_schedule` field~~ — confirmed in Phase 4 (`useOnboardingDailySchedule.test.tsx`'s "is a no-op for schemas without a daily_schedule field").
