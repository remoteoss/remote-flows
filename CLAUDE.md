...
npm run verify:contract-details -- --country=ESP --version=7 [--pass=required|full|both]  # drive the headless useOnboarding hook through contract_details end to end against the sandbox gateway; needs example/node_modules and .env.sandbox (see scripts/verify-contract-details-version/)
```
